...
class DirectField(BaseField):
    ...
    def fft(self):
        ...
        if self.rank == 1:
            data = self.fft_object(self) * self.grid.dV
        else:
    ...
    def get_cut(
        ...
        return DirectField(grid=cut_grid, data=values)
    ...
    def ifft(self, check_real=False, force_real=False):
        ...
        if self.rank == 1:
            ...
            if self.cplx or np.all(
                ...
                data = np.empty(nr)
            for i in range(self.rank):
                data[i] = self.ifft_object(self[i], **fft_kwargs) / direct_grid.dV
        if check_real:
            if np.isclose(np.imag(data), 0.0, atol=1.0e-16).all():
                data = np.real(data)
        if force_real:
            data = np.real(data)
